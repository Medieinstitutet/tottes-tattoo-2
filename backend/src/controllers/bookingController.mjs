...
export const createBooking = async (req, res, next) => {
  try {
    ...
    if (!name || !email || !date || !time || !type || !duration) {
      throw new ErrorResponse('All fields except file are required.', 400);
    }
    ...
    if (!isNotLunchHour(time)) {
      throw new ErrorResponse(
        'Bookings are closed for lunch between 12:00 and 13:00.',
        400
      );
    }
    ...
    if (bookedTattooers.size >= 5) {
      throw new ErrorResponse(
        'All tattooers are booked for this time slot.',
        400
      );
    }
    ...
    if (availableTattooers.length === 0) {
      throw new ErrorResponse(
        'No tattooers available for this time slot.',
        400
      );
    }
    ...
    const assignedTattooer =
      availableTattooers[Math.floor(Math.random() * availableTattooers.length)];
    ...
    const booking = {
      ...
      filePath: file?.path || null, // valfri bild
    };
...
export const getAvailableSlots = async (req, res, next) => {
  try {
    ...
    if (!isWeekday(date)) {
      throw new ErrorResponse(
        'Bokningar är endast tillgängliga måndag till fredag',
        400
      );
    }
    ...
    workHours.forEach(({ start, end }) => {
      ...
      while (current < endTime) {
        ...
        const bookingsAtTime = bookings.filter(
          (b) => b.date === date && b.time === slotTime
        );
        if (bookingsAtTime.length < 5) {
          // Mindre än 5 tatuerare bokade
  ...
  try {
    ...
    if (updates.time) {
      ...
      if (!isWithinWorkingHours(updates.time)) {
        throw new ErrorResponse(
          'Bokningar måste vara mellan 09:00 och 18:00',
          400
        );
      }
      if (!isNotLunchHour(updates.time)) {
        throw new ErrorResponse(
          'Bokningar är stängda för lunch mellan 12:00 och 13:00',
          400
        );
      }
    }
    ...
    if (updates.duration) {
      ...
      if (isNaN(durationNum) || durationNum < 60 || durationNum > 480) {
        throw new ErrorResponse(
          'Varaktighet måste vara mellan 60 och 480 minuter',
          400
        );
      }
    ...
    if (updates.time || updates.date) {
      ...
      const overlappingBookings = existingBookings.filter((b) => {
        if (b.date !== checkDate || parseInt(b.id) === parseInt(id))
          return false;
      ...
      const bookedTattooers = new Set(
        overlappingBookings.map((b) => b.tattooer)
      );
      if (bookedTattooers.size >= 5) {
