...
export default function BookingCard({ booking, onDelete, onEdit }) {
  ...
  return (
    <Card bgColor={bgColor}>
      ...
      <Field><Label>Kund:</Label> {booking.customer}</Field>
      <Field><Label>Typ av bokning:</Label> {booking.type}</Field>
      <Field><Label>Datum & tid:</Label> {booking.date} – {booking.time}</Field>
      <Field><Label>Tatuerare:</Label> {booking.employee}</Field>
      <Field><Label>Email:</Label> {booking.email}</Field>

      {booking.phone && <Field><Label>Telefon:</Label> {booking.phone}</Field>}
      <Field><Label>Längd:</Label> {booking.duration}</Field>
      {booking.description && (
        <Field><Label>Beskrivning:</Label> {booking.description}</Field>
      )}
  ...
  (/\.(jpg|jpeg|png)$/i.test(booking.referenceImage)) && (
    <Field>
      ...
      <div style={{
        ...
        <a
          href={fullImageURL}
          download={booking.referenceImage.split('/').pop() || 'referensbild.jpg'}
          style={{
            fontSize: '0.85rem',
            color: '#d4af37',
            textDecoration: 'underline',
      ...
      <ButtonRow>
        ...
        <ActionButton danger onClick={() => onDelete(booking.id)}>Ta bort</ActionButton>
      </ButtonRow>
    </Card>
  );
}
